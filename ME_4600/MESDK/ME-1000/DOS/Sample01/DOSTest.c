...
#define DEVICE_ID_B 0x100B
        ...
        else
        {
            ...
            */
            ...
            printf("Reset card ... (all ports are inputs) \n\n");
            {
                WriteDWord(gDevInfo.wBaseIO+0x00, 0x00);
                WriteDWord(gDevInfo.wBaseIO+0x04, 0x00);
                WriteDWord(gDevInfo.wBaseIO+0x08, 0x00);
                WriteDWord(gDevInfo.wBaseIO+0x0C, 0x00);

                WriteDWord(gDevInfo.wBaseIO+0x10, 0x00);    // Set all ports to input
            }
            ...
            printf("Reading ports ... \n\n");
            {
                ...
                Value = ReadDWord(gDevInfo.wBaseIO+0x00);
                printf("\tPort 0: 0x%08x\n", Value);
                Value = ReadDWord(gDevInfo.wBaseIO+0x04);
                printf("\tPort 1: 0x%08x\n", Value);
                Value = ReadDWord(gDevInfo.wBaseIO+0x08);
                printf("\tPort 2: 0x%08x\n", Value);
                Value = ReadDWord(gDevInfo.wBaseIO+0x0C);
                printf("\tPort 3: 0x%08x\n", Value);
            }


            printf("Write and read back as output ... \n");
            printf("Be careful not to cause a short circuit!\n");
            printf("(thats why the following source code is commented out)\n");
            if (0)
            {
                int Value;

                WriteDWord(gDevInfo.wBaseIO+0x10, 0x01);    // Set port 1 to output

                Value = 0xAA;

                WriteDWord(gDevInfo.wBaseIO+0x00, Value);   // Set any value to the port
                printf("\tSet Port 0 with: 0x%08x\n", Value);

                Value = ReadDWord(gDevInfo.wBaseIO+0x00);   // You can read back the output word
                printf("\tRead Port 0: 0x%08x\n", Value);
            }
        }
    }
}
