...
    /*--- now we can play --------------------------------------*/
    ...
    */
    ...
    /*--- set values on bipolar mode voltage out ------*/
    {
        int iValue;
        int iChannel;
        int iCharacter;

        printf("write single values in bipolar mode ...\n");

        // set bipolar mode (not needed after hardware reset)
        outport(gDI.wBaseIO + 0x20, 0x00);

        for (iChannel=0; iChannel<16; iChannel++)
        {
            iValue = 0x0FFF;    // Write 10 V

            printf("\r write %03x to channel %d\t\t\t\t ", iValue, iChannel);

            outport(gDI.wBaseIO + 0x00 + (iChannel*2), iValue);
            ...
            iValue = 0x0000;    // Write -10 V

            printf("\r write %03x to channel %d\t\t\t\t \n", iValue, iChannel);
    ...
    /*--- set values on unipolar mode voltage out ------*/
    {
        ...
        // set unipolar mode
        outport(gDI.wBaseIO + 0x20, 0xFFFF);
    ...
    /*--- set values on simultaneous mode ------*/
    {
        int iCharacter;
        int iChannel;
        int iConfValue;

        // set bipolar mode (for instance)
        outport(gDI.wBaseIO + 0x20, 0x0000);

        // set DACs in simultaneous mode
        // DAC1: 01 / DAC2: 02 / DAC3: 04 / DAC4: 08 or the combinations
        outport(gDI.wBaseIO + 0x24 , 0x0008);

        // set a value
        outport(gDI.wBaseIO + 0x18, 0x0FFF);
    ...
    /*--- set values current out ------*/
    {
        ...
        // set unipolar/bipolar ignored
        // outport(gDI.wBaseIO + 0x20, 0xFFFF);

        for (iChannel=0; iChannel<8; iChannel++)
        {
            sleep(1);
            outport(gDI.wBaseIO + 0x00 + (iChannel*2), 0x00);
        }

        sleep(1);
        outport(gDI.wBaseIO + 0x22, 0x0000);
        sleep(1);
        outport(gDI.wBaseIO + 0x26, 0x00FF);
        sleep(1);
