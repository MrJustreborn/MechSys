...
int main(int argc, char* argv[])
{
    ...
    {
        printf(":------:---------------:------------:\n");
        printf(":Board : Serial Number :  VEC Model :\n");
        printf(":------:---------------:------------:\n");

        for (iBrdIdx=0; iBrdIdx<ME_MAX_DEVICES; iBrdIdx++)
        {
            if (me3000GetSerialNumber(iBrdIdx, &iSrlNmbr))
            {
                if (me3000GetVECModel(iBrdIdx, &iVecModel))
                {
                    printf(":  %3d :      %08X :          %d :\n", iBrdIdx, iSrlNmbr, iVecModel);
                    if (iVecModel != 3)
                        bUpgrade = TRUE;
                }
                else
                {
                    ...
                    if (strstr(error_text, "not supported"))
                        printf(":  %3d :      %08X : not needed :\n", iBrdIdx, iSrlNmbr);
                    else
                    if (strstr(error_text, "No valid licence"))
                    {
                        printf(":  %3d :      %08X :  not found :\n", iBrdIdx, iSrlNmbr);
                        bWarning = TRUE;
                    }
                    else
        ...
        printf("Press 'y' to register a additional licence key\n");
